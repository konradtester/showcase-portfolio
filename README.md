# SDET & AI Product Engineer | Professional Showcase

## 👋 Introduction

Welcome to my technical portfolio. This repository highlights my expertise in building and testing scalable AI-powered SaaS platforms.

> [!IMPORTANT]
> To protect proprietary business logic and private codebases, all project names and specific implementation details have been anonymized. The core architecture and engineering patterns shown here accurately reflect my production-grade standards.

## 📁 Repository Structure

### [🚀 Projects Showcase](./projects)

High-level overview of complex systems I've architected and maintained:

- **[EventCore AI](./projects/EventCore_AI.md)**: Real-time event management platform with advanced RAG implementation.
- **[SafeGuard AI](./projects/SafeGuard_AI.md)**: Enterprise safety and emergency management system with critical RBAC protocols.
- **[EduStream AI](./projects/EduStream_AI.md)**: Intelligent lecture simplification tool leveraging LLMs and STT.

### [💻 Code Snippets](./snippets)

Proof-of-style snippets demonstrating:

- **FastAPI**: Dependency Injection, Pydantic validation, and service-oriented architecture.
- **LangChain/LangGraph**: Orchestration of self-correcting AI agents.

### [🏗️ Test Automation Strategy](./strategy)

Comprehensive guides and CI/CD pipeline examples for:

- Playwright + Java/TypeScript.
- Cucumber (BDD) vs Native Playwright.
- Multi-environment testing in GitLab/GitHub Actions.

---

## 🛠️ Core Technology Stack

- **Languages**: Python, TypeScript, Java, Dart (Flutter).
- **Backend**: FastAPI, SQL Alchemy, PostgreSQL, Redis.
- **Frontend**: Flutter, React.
- **AI/ML**: LangChain, LangGraph, Qdrant (Vector DB), OpenAI/Gemini APIs.
- **DevOps/Testing**: Playwright, CI/CD (GitLab/GitHub), Docker.

---

_Contact me for a deeper dive into any of these concepts._
